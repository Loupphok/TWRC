...
def get_all_nadeo_uploads():
    ...
    # Get all replay data
    base_url = "https://tmuf.exchange/api/replays"
    ...
    for track_name, track_id in zip(track_info['TrackName'], track_info['TrackID']):
        ...
        index_wr = 1
        all_uploads.append([current_wr[1], current_wr[4].strftime("%d/%m/%Y"), current_wr[2], current_wr[0], 0])
        for run in current_map[1:]:
            if run[3] < current_wr[3]:
                current_wr = run
                index_wr += 1
                all_uploads.append([run[1], run[4].strftime("%d/%m/%Y"), run[2], run[0], 0])
                
    # Sort and save replay data
    # all_uploads = sorted(all_uploads, key=lambda k: k[4])
    with open('otherTests/FetchingWRs/TMUF_fetch.txt', 'w') as file:
